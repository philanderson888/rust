...
fn main() {
    ...
    println!("... we can also use the `unwrap_or` method to provide a default value ...");

    let score = scores.get(&team_name).unwrap_or(&0);

    println!("The score for the Blue team is: {}", score);

    println!("... we can iterate over the key-value pairs in a hashmap using a for loop ...");
    ...
    println!("... we can update a value in a hashmap using the insert method ...");
    println!("... insert adds or overwrites the key and value ...");

    scores.insert(String::from("Blue"), 25);
    println!("scores: {:?}", scores);

    println!("... `entry` will only add a value if the key does not exist.");
    ...
    println!("... in this example we continually update the hashmap count for each word");
    ...
    println!("word count hashmap is : {:?}", word_count_hashmap);

    println!("... we see that each loop we check if an entry exists with `.entry` and if not, inserts a new value 0");
